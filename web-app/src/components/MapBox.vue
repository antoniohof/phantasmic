<template>
  <div class="map">
    ...
    <div v-if="creatingStory" class="calculation-box" :class="{ 'blink': showTooltip }">
      <p>Use the draw tool on the top to mark the area of the sighting, then press enter</p>
...
export default {
  ...
  mounted () {
    if (this.forcedZoom) {
      this.initialZoom = this.forcedZoom
    }
  },
  data () {
    return {
      mapStyle: "mapbox://styles/antoniohof/ckjstmp5e7z0919oa02xki51z",
      initialCenter: [],
      initialZoom: 2,
      mapbox: null,
      lastSelected: -1,
      ...
      if (this.locked) {
        ...
        setTimeout(() => {
          this.mapbox.jumpTo({
            ...
            essential: true,
            zoom: story.zoom ? (story.zoom - 2) : 4
          })
        }, 200)
      }
